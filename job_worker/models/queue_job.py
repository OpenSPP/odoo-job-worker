...
class QueueJob(models.Model):
    ...
    def _cascade_children_on_parent_success(self):
        """Release waiting parent_id children when this parent completes.

        on_done children (run_on_failure=False) move waiting → pending.
        on_error children (run_on_failure=True) are cancelled.
        """
        self.ensure_one()
        now = fields.Datetime.now()
    ...
    def _cascade_children_on_parent_failure(self):
        """Cascade waiting parent_id children when this parent fails.

        on_done children (run_on_failure=False) cascade to failed.
        on_error children (run_on_failure=True) are promoted to pending.
        """
        self.ensure_one()
        for child in self.child_ids.filtered(lambda c: c.state == "waiting"):
            if child.run_on_failure:
